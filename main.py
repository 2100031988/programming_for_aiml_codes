...
# A compilation translates an entire program into machine code beforehand and a interpretation translates 
# and executes code line-by-line at runtime
# Cpython is a combination of C and python, Jpython is a combination of Java and Python whereas Pypy is python written
# within a python like restricted python (a subset of python)
...
# Print function() begins its output from a new line each time it starts its execution and can 
# accepts any number of arguments

# A function (it is inbulit) has particuluary two task : cause some effect and evaluate a value 
# (return it as function's result)

# function_name(argument) checks if the name is legal, then checks if we have invoked it correctly like kept correct arguments,
# next jumps to the desired function and executes the code.
...
# End keyword argument determines the characters the print() function sends to the output once it reaches the end of its positional 
# arguments or in another way the end keyword is same like /n (it's task) and it's syntax is end=" "
...
# A literal is data whose values are determined by the literal itself and we use them to encode data and to put them into your code.
# For example 1234 is a literal whereas c is not a literal.
...
# An octal number is representated by 00 or 0o this means that the digits may be contained from 0 to 7 only. Example: 0o123 is 
# converted as 83 and this can be done by python's print function

# A hexadecimal number is that which has a (decimal) value equal to 291.

# 4 is an integer whereas 4.0 is a floating point number and sometimes the python compiler uses this literal as we try 
# to print(0.0000000000000000000001) it gives 1e-22 where e is the exponet and -22 is the power of 10.
...
# Here the operators are mathematical ones (+, -, *, /, //, %, **) and the exponentiation operator( ** ) accepts 
# left argument as base and right as exponent.
...
# Binding of the operator determines the order of computations performed by some operators with equal priority, put 
# side by side in one expression.

# Example: print(9 % 6 % 2) will print 9%6 as 3 and then 3%2 as 1 and it's always left side binding only. 
# print(2 ** 2 ** 3) prints 64 (2**2 and then 4**3)
...
# 'try', 'while', 'with', 'yield'] they are "reserved keywords" or they are predefined

# Shortcut operators or compound assignment operators are like x=x+1 as x++, x*=2 as x=x*2 and so on and remember that each 
# variable must have a unique name as identifier.
...
# Replication operator happens when we apply it to a string and number or a number and a string. Ex-- "James" * 
# 3 gives "JamesJamesJames" and 3 * "an" gives output as "ananan". A number less than or equal to 0 prints empty string.

# The assignment operator (=) assigns value to a variable. Ex - a=b and the equals to operator (==) compares and returns
# boolean values like is the value on the left hand side is equal to the right hand side or not?
...
# If-else (condtional statements) checks if the value satisfy the condition or not. The syntax is if(statement>statement2): 
# print("True") else: print("False") and the use of this is known as nesting.
...
# The built-in function [max() and min()] is used to find the largest and smallest of all numbers. 
# Example: max(11,12,23) will print 23 and 11 as resp.
